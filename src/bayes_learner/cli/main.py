...
import importlib
import pkgutil
from bayes_learner.cli import commands

def main():
    parser = argparse.ArgumentParser(prog="bayes-learner", description="Bayes-learner CLI")
    subparsers = parser.add_subparsers(dest="command", required=True)
    for _, module_name, is_pkg in pkgutil.iter_modules(commands.__path__):
        if is_pkg or module_name.startswith("_"):
            continue
        try:
            module = importlib.import_module(f"bayes_learner.cli.commands.{module_name}")
